...
import { globEscape, qid, toSqlValue } from "./utils";
...
function numGuard(field: string, op: string, value: number): PushResult {
	const f = qid(field);
	return {
...
function textGuard(field: string, op: string, value: string): PushResult {
	const f = qid(field);
	return {
...
function rangeOp(field: string, value: any, op: string): PushResult | null {
	if (typeof value === "number" && Number.isFinite(value))
		return numGuard(field, op, value);
	if (typeof value === "string") return textGuard(field, op, value);
	return null;
}

function buildInComposite(
	field: string,
	list: any[],
): {
	sql: string;
	...
	if (numbers.length > 0) {
		groups.push(
			`(typeof(${f}) ${NUMERIC_TYPEOF} AND ${f} IN (${numbers.map(() => "?").join(",")}))`,
		);
		values.push(...numbers);
	}
	if (strings.length > 0) {
		groups.push(
			`(typeof(${f}) = 'text' AND ${f} IN (${strings.map(() => "?").join(",")}))`,
		);
		values.push(...strings);
	}
	if (others.length > 0) {
...
export const PUSHABLE_OPS: Record<string, PushFn> = {
