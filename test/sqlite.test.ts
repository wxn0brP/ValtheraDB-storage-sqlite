...
describe("SQLiteValthera", () => {
    ...
    test("6. round-trips booleans, nulls and numeric-like strings", async () => {
        const { db } = setup("CREATE TABLE docs (_id TEXT PRIMARY KEY, flag INT, note TEXT, code TEXT)");
        const docs = db.c("docs");

        await docs.add({ _id: "d1", flag: true, note: "x", code: "007" });
        const doc = await docs.findOne<any>({ _id: "d1" });

        expect(doc.flag).toBe(true);
        expect(doc.note).toBe("x");
        expect(doc.code).toBe("007");

        const byBool = await docs.find<any>({ flag: true });
        expect(byBool).toHaveLength(1);

        const byStr = await docs.find<any>({ code: "007" });
        expect(byStr).toHaveLength(1);
    });

    test("7. preserves foreign binary blobs", async () => {
        const { sqlDB, db } = setup("CREATE TABLE bins (_id TEXT PRIMARY KEY, data BLOB)");
        const binary = new Uint8Array([0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a, 1, 2, 3]);
        sqlDB.prepare("INSERT INTO bins (_id, data) VALUES (?, ?)").run("b1", binary);

        const bin = await db.c("bins").findOne<any>({ _id: "b1" });
    ...
    test("8. search-by-null matches explicit nulls only", async () => {
        ...
        await items.add({ _id: "a", val: null });
        await items.add({ _id: "b", val: "text" });
        await items.add({ _id: "c" });

        const found = await items.find<any>({ val: null });
        expect(found).toHaveLength(1);
    ...
    test("9. $exists pushdown matches missing columns", async () => {
        const { db } = setup("CREATE TABLE items (_id TEXT PRIMARY KEY, temp TEXT)");
        const items = db.c("items");

        await items.add({ _id: "a", temp: "hot" });
        await items.add({ _id: "b" });

        const existing = await items.find<any>({ $exists: { temp: true } });
        expect(existing.map(d => d._id)).toEqual(["a"]);

        const missing = await items.find<any>({ $exists: { temp: false } });
        expect(missing.map(d => d._id)).toEqual(["b"]);
    });

    test("10. empty $or and array search match nothing", async () => {
        const { db } = setup("CREATE TABLE items (_id TEXT PRIMARY KEY, name TEXT)");
        const items = db.c("items");
        await items.add({ _id: "a", name: "A" });

        expect(await items.find({ $or: [] })).toHaveLength(0);
        expect(await items.find({} as any)).toHaveLength(1);
        expect(await items.find(["a"] as any)).toHaveLength(0);
    });

    test("11. logical operators $and/$or/$not", async () => {
        const { db } = setup("CREATE TABLE items (_id TEXT PRIMARY KEY, val INT)");
        const items = db.c("items");
        for (const v of [1, 5, 10]) await items.add({ _id: `i${v}`, val: v });

        const or = await items.find<any>({ $or: [{ val: 1 }, { val: 10 }] });
        expect(or.map(d => d._id).sort()).toEqual(["i1", "i10"]);

        const and = await items.find<any>({ $and: [{ $gt: { val: 0 } }, { $lt: { val: 6 } }] });
        expect(and.map(d => d._id).sort()).toEqual(["i1", "i5"]);

        const not = await items.find<any>({ $not: { $gte: { val: 5 } } });
        expect(not.map(d => d._id)).toEqual(["i1"]);
    });

    test("12. range and list operators", async () => {
        const { db } = setup("CREATE TABLE items (_id TEXT PRIMARY KEY, val INT, tag TEXT)");
        const items = db.c("items");
        await items.add({ _id: "a", val: 1, tag: "x" });
        await items.add({ _id: "b", val: 5, tag: "y" });
        await items.add({ _id: "c", val: 9, tag: "z" });

        expect((await items.find<any>({ $between: { val: [2, 8] } })).map(d => d._id)).toEqual(["b"]);
        expect((await items.find<any>({ $in: { val: [1, 9] } })).map(d => d._id).sort()).toEqual(["a", "c"]);
        expect((await items.find<any>({ $nin: { val: [1, 9] } })).map(d => d._id)).toEqual(["b"]);
        expect((await items.find<any>({ $startswith: { tag: "y" } })).map(d => d._id)).toEqual(["b"]);
        expect((await items.find<any>({ $endswith: { tag: "z" } })).map(d => d._id)).toEqual(["c"]);
    });

    test("13. $inc on missing column sets delta", async () => {
        const { db } = setup("CREATE TABLE items (_id TEXT PRIMARY KEY, count INT)");
        const items = db.c("items");
        await items.add({ _id: "a" });

        const updated = await items.updateOne<any>({ _id: "a" }, { $inc: { count: 3 } });
        expect(updated.count).toBe(3);

        const found = await items.findOne<any>({ _id: "a" });
    ...
    test("14. $unset removes field from database", async () => {
        const { db } = setup("CREATE TABLE items (_id TEXT PRIMARY KEY, temp TEXT)");
        const items = db.c("items");
        await items.add({ _id: "a", temp: "value" });

        const updated = await items.updateOne<any>({ _id: "a" }, { $unset: { temp: "" } });
    ...
    test("15. update to undefined nullifies column", async () => {
        ...
        const items = db.c("items");
        await items.add({ _id: "a", opt: "yes" });

        const updated = await items.updateOne<any>({ _id: "a" }, { opt: undefined });
        expect(updated.opt).toBeUndefined();

        const found = await items.findOne<any>({ _id: "a" });
        expect(updated.opt === undefined || !("opt" in found) || found.opt === null).toBe(true);
    });
    ...
    test("16. add creates missing columns dynamically", async () => {
        ...
        await items.add({ _id: "a", extra: 42 });
        const found = await items.findOne<any>({ _id: "a" });
    ...
    test("17. concurrent queries share prepared statements safely", async () => {
        const { db } = setup("CREATE TABLE items (_id TEXT PRIMARY KEY, val INT)");
        const items = db.c("items");
        for (let i = 0; i < 20; i++) await items.add({ _id: `i${i}`, val: i });

        const results = await Promise.all(
            Array.from({ length: 10 }, (_, k) => items.findOne<any>({ _id: `i${k}` }))
        );
